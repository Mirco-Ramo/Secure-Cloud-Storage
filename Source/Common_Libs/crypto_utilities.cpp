...
int symm_decrypt(unsigned char* enc_buf, unsigned short enc_size, unsigned char* session_key, unsigned char* IV, unsigned char*& clear_buf, unsigned short& clearlen){
    ...
    //Inizializzo il contesto per la deciratura
    ctx = EVP_CIPHER_CTX_new();
    if(!ctx){
        cerr << "Error: EVP_CIPHER_CTX_new returned NULL\n";
        return FATAL_ERR;
    }
    //Creo il buffer che conterrÃ  il testo decifrato
    ...
    /*si assume che sia sufficiente una sola update in quanto il server non
        ha necessitÃ  di cifrare grandi quantitÃ  di dati*/
